...
use dynomite::dynamodb::{QueryInput, QueryOutput, QueryError};
use uuid::Uuid;
...
pub trait UserQuoteRepository {
    ...
    async fn get_quote_by_id(&self, message_id: MessageId, channel_id: ChannelId) -> Option<UserQuote>;
...
impl UserQuoteRepository for UserQuoteDDBRepository {
    ...
    async fn get_quote_by_id(&self, message_id: MessageId, channel_id: ChannelId) -> Option<UserQuote> {
        ...
        let query = self.client.query(
            QueryInput {
                ...
                key_condition_expression: Some("channel_id = :channel_id AND message_id = :message_id".to_string()),
                limit: Some(1),
                table_name: self.table_name.clone(),
    ...
    async fn get_random_quote(&self, server_name: &str) -> Option<UserQuote> {
        ...
        let gt_query = self.client.query(
            QueryInput {
                expression_attribute_values: Some(key_mapping.clone()),
                key_condition_expression: Some("server_name = :server_name AND sort_id > :sort_id".to_string()),
                limit: Some(10),
                table_name: self.table_name.clone(),
                ..QueryInput::default()
            }
        ...
        let lt_query = self.client.query(
            QueryInput {
        ...
        let mut final_results: Vec<UserQuote>= gt_result.iter_mut().filter_map(|mapping| UserQuote::from_attrs(mapping).ok()).collect();

        final_results.sort();
        final_results.dedup();
        let retval= final_results.choose(&mut rand::thread_rng()).unwrap().clone();
        Some(retval)
    }
    ...
    async fn get_quotes_for_user(&self, user_id: UserId) -> Vec<UserQuote> {
        ...
        match query {
            Ok(result) => {
                result.items.unwrap().iter_mut().filter_map(|entry| {
                    UserQuote::from_attrs(entry).ok()
                }).collect()
            }
            Err(_) => {
                Vec::new()
